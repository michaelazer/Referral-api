...
module.exports = {
    getAnalyticsData: () => {
        return new Promise(async (resolve, reject) => {
            // Quick fix: return empty analytics for now
            resolve({
                participantStats: { total_participants: 0, total_conversions: 0, active: 0, invited: 0, flagged: 0 },
                customerStats: { total_customers: 0, paying_customers: 0, flagged_customers: 0 },
                mediaStats: { fb_shares: 0, twitter_shares: 0, linkedin_shares: 0, whatsapp_shares: 0, email_shares: 0 },
                clickStats: { total_clicks: 0 },
                totalRevenue: 0,
                awaitingPayouts: 0,
                redeemedCredits: 0
            });
            return; // Skip the hanging queries for now
            ...
            async.parallel({
                ...
                mediaStats:function(callback){
                    let stats = {};
                    stats.fb_shares = stats.twitter_shares = stats.linkedin_shares = stats.whatsapp_shares = stats.email_shares = 0;
                    urls.map(url => {
                        stats.fb_shares += url.data.fb_shares;
                        stats.twitter_shares += url.data.twitter_shares;
                        stats.linkedin_shares += url.data.linkedin_shares;
                        stats.whatsapp_shares += url.data.whatsapp_shares;
                        stats.email_shares += url.data.email_shares
                    })
                    callback(null,stats);
                },
